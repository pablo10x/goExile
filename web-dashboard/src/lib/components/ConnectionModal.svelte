<script lang="ts">
	...
	let { isOpen = $bindable(), onClose = undefined } = $props<{
		isOpen: boolean;
		onClose?: () => void;
	}>();
	...
	>
		...
		>
			...
			<div class="p-8 space-y-6">
				...
				<div class="space-y-2">
					...
					<div class="relative group">
						<div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
							<Globe
								class="h-4 w-4 text-slate-500 group-focus-within:text-sky-400 transition-colors"
							/>
						</div>
						<input
							id="serverUrl"
				...
				<div
					class="flex items-center gap-2 text-[10px] text-slate-500 font-bold uppercase tracking-widest bg-slate-950/50 p-3 rounded-xl border border-white/5"
				>
			...
			<div
				class="px-8 py-6 bg-slate-950 border-t border-white/5 flex items-center justify-between gap-4"
			>
				...
				<div class="flex gap-3">
					...
					<Button onclick={save} loading={isSaving} variant="primary" size="md">Save & Sync</Button>
				</div>
			</div>
		</div>
	</div>
