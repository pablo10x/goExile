...
<div
	class="bg-white dark:bg-slate-950 border-r border-slate-200 dark:border-slate-800 flex flex-col h-full transition-all duration-300 ease-in-out {isSidebarOpen
		? 'w-full'
		: 'w-16 bg-transparent border-none'}"
>
	<!-- Header -->
	<div
		class="p-3 border-b border-slate-200/50 dark:border-slate-800/50 flex flex-col gap-3 relative shrink-0"
	>
		<div class="flex justify-between items-center h-8">
			{#if isSidebarOpen}
				<div
					class="flex items-center gap-2 text-slate-100 font-bold tracking-tight animate-in fade-in slide-in-from-left-2"
				>
		...
		{#if isSidebarOpen}
			<div class="relative group">
				<Search
					class="w-3.5 h-3.5 absolute left-3 top-2.5 text-slate-500 group-focus-within:text-indigo-400 transition-colors"
				/>
				<input
					type="text"
	...
	{#if isSidebarOpen}
		<div class="flex-1 overflow-y-auto overflow-x-hidden p-2 space-y-1 custom-scrollbar">
			{#if filteredData.length === 0}
				<div
					class="flex flex-col items-center justify-center h-32 text-slate-600 text-xs animate-in fade-in"
				>
			...
			{#each filteredData as schema (schema.name)}
				<div class="group/schema" transition:slide|local={{ duration: 200, easing: quintOut }}>
					...
					>
						...
						{:else}
							<Folder
								class="w-4 h-4 text-slate-500 group-hover/row:text-slate-500 dark:text-slate-400"
							/>
						{/if}

						<span
							class="truncate font-medium {schema.isOpen
								? 'text-indigo-100'
								: 'text-slate-500 dark:text-slate-400'} flex-1"
						>
							{schema.name}
						</span>
					...
					{#if schema.isOpen}
						...
						>
							{#if schema.tables.length === 0}
								<div class="px-3 py-1 text-[10px] text-slate-600 italic pl-6">Empty schema</div>
							{/if}
