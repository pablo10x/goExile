...
<div
	class="bg-slate-900/40 border-r border-slate-800 flex flex-col h-full transition-all duration-300 ease-in-out backdrop-blur-sm {isSidebarOpen
		? 'w-full'
		: 'w-16 bg-transparent border-none'}"
>
	<!-- Header -->
	<div
		class="p-4 border-b border-slate-800 flex flex-col gap-4 relative shrink-0"
	>
		<div class="flex justify-between items-center h-10">
			{#if isSidebarOpen}
				...
				<div class="flex gap-1">
					<button
						...
						class="p-2 hover:bg-slate-800 text-slate-500 hover:text-indigo-400 transition-all rounded-xl"
						title="New Schema"
					>
						<Plus class="w-4 h-4" />
					</button>
					<button
						...
						class="p-2 hover:bg-slate-800 text-slate-500 hover:text-white transition-all rounded-xl"
						title="Collapse"
					>
			...
			{:else}
				<button
					onclick={onToggleSidebar}
					class="w-10 h-10 flex items-center justify-center bg-slate-900/50 border border-slate-800 text-slate-500 hover:text-indigo-400 transition-all mx-auto rounded-xl"
					title="Expand"
				>
		...
		{#if isSidebarOpen}
			<div class="relative group">
				<Search
					class="w-3.5 h-3.5 absolute left-3 top-1/2 -translate-y-1/2 text-slate-500 group-focus-within:text-indigo-400 transition-colors"
				/>
				<input
					type="text"
					bind:value={filterText}
					placeholder="Filter objects..."
					class="w-full bg-slate-950/40 border border-slate-800 py-2 pl-9 pr-8 text-xs font-bold uppercase tracking-widest text-slate-300 placeholder:text-slate-700 focus:border-indigo-500/50 outline-none transition-all rounded-xl"
				/>
				{#if filterText}
					<button
						onclick={() => (filterText = '')}
	...
	{#if isSidebarOpen}
		<div class="flex-1 overflow-y-auto overflow-x-hidden p-2 space-y-1 custom-scrollbar">
			{#if filteredData.length === 0}
				<div
					class="flex flex-col items-center justify-center h-32 text-slate-700 text-[10px] font-bold uppercase tracking-[0.3em] animate-in fade-in"
				>
			...
			{#each filteredData as schema (schema.name)}
				<div class="group/schema" transition:slide|local={{ duration: 200, easing: quintOut }}>
					...
					>
						<div
							class="w-4 h-4 flex items-center justify-center text-slate-600 transition-transform duration-200 {schema.isOpen
						...
						{:else}
							<Folder
								class="w-4 h-4 text-slate-600 group-hover/row:text-slate-400 transition-colors"
							/>
						{/if}
						...
						<div class="flex gap-1 opacity-0 group-hover/row:opacity-100 transition-opacity ml-auto">
							<button
								...
								class="p-1 hover:bg-indigo-500/10 text-slate-600 hover:text-indigo-400 transition-colors rounded-lg"
								title="Create Table"
							>
							...
							<button
								...
								class="p-1 hover:bg-red-500/10 text-slate-600 hover:text-red-400 transition-colors rounded-lg"
								title="Drop Schema"
							>
					...
					{#if schema.isOpen}
						<div
							class="ml-3 pl-3 border-l border-slate-800 space-y-0.5 mt-1 mb-2"
							transition:slide|local={{ duration: 150 }}
						>
							{#if schema.tables.length === 0}
								<div class="px-3 py-2 text-[9px] font-medium text-slate-700 uppercase italic tracking-widest">No_Tables_Mapped</div>
							{/if}
								...
								>
									<Table
										class="w-3.5 h-3.5 {selectedSchema === schema.name && selectedTable === table
											? 'text-indigo-400'
											: 'text-slate-700 group-hover/table:text-slate-500'}"
									/>
									...
									>
										<button
											...
											class="p-1 hover:bg-slate-800 text-slate-600 hover:text-indigo-400 transition-all rounded-lg"
										...
										<button
											...
											class="p-1 hover:bg-red-500/10 text-slate-600 hover:text-red-400 transition-all rounded-lg"
											title="Drop Table"
										>
