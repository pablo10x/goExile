...
<div class="relative">
    ...
    >
        ...
        {#if $notifications.length > 0}
            <span class="absolute top-1 right-1 w-2.5 h-2.5 bg-red-500 rounded-full border-2 border-slate-900 animate-ping"></span>
    ...
    {#if isNotificationPanelOpen}
        ...
        >
            <div class="p-3 border-b border-slate-700/50 flex justify-between items-center bg-slate-800/30">
                ...
                <button onclick={() => notifications.clearHistory()} class="text-xs text-blue-400 hover:text-blue-300">Clear</button>
            </div>
            <div class="max-h-80 overflow-y-auto p-2 space-y-2">
                {#if $history && $history.length > 0}
                    {#each $history as note (note.id)}
                        <div class="flex gap-3 p-2 rounded-lg bg-slate-800/30 border border-slate-700/30 hover:bg-slate-800/50 transition-colors">
                            <div class="mt-0.5">
                                {#if note.type === 'success'} <CheckCircle class="w-4 h-4 text-emerald-400" />
                                {:else if note.type === 'error'} <XCircle class="w-4 h-4 text-red-400" />
                                {:else if note.type === 'warning'} <AlertCircle class="w-4 h-4 text-orange-400" />
                                {:else} <Info class="w-4 h-4 text-blue-400" /> {/if}
                            </div>
