...
<div
	class="border border-neutral-800 rounded-none glass-panel overflow-hidden mb-2 hover:border-rust/40 transition-all duration-500 shadow-lg group/row relative bg-neutral-900/40"
...
>
		...
		<div class="absolute left-0 top-0 bottom-0 flex flex-col justify-center px-2 pointer-events-none overflow-hidden opacity-20">
            <div class="animate-binary-slide flex flex-col gap-0.5 will-change-transform">
                {#each [...binaryLines, ...binaryLines] as line}
                    <div class="font-mono text-[6px] text-rust-light whitespace-nowrap leading-tight mask-fade-right">
                        {line}
                    </div>
                {/each}
            </div>
	...
	<div class="flex flex-col sm:flex-row sm:items-center gap-4 cursor-pointer hover:bg-rust/5 transition-all relative z-10">
		...
		>
			<!-- Chevron -->
			<div
				class="text-neutral-600 transform transition-transform duration-500 {expanded
			...
			<div class="flex flex-col min-w-[160px] ml-4 sm:ml-8">
				<div class="flex items-center gap-2">
					<span class="text-[9px] font-jetbrains font-black uppercase tracking-tighter text-neutral-600">INSTANCE</span>
				...
				<div class="flex items-center gap-2 mt-1">
					<span class="text-[8px] font-jetbrains font-bold text-neutral-700 uppercase">Port:</span>
			...
			<div class="sm:w-36">
				{#if instance.status === 'Running'}
					<div class="flex items-center gap-2.5 text-emerald-500 bg-emerald-500/5 px-3 py-1 border border-emerald-500/20 rounded-none w-fit shadow-[0_0_15px_rgba(16,185,129,0.05)]">
				...
				{:else}
					<div class="flex items-center gap-2.5 text-neutral-500 bg-neutral-950/40 px-3 py-1 border border-neutral-800 rounded-none w-fit">
			...
			<div class="flex flex-col hidden md:flex">
				<span class="text-[8px] font-jetbrains font-black text-neutral-700 uppercase tracking-widest">Version</span>
				<div class="flex items-center gap-2">
					<span class="text-[10px] font-jetbrains font-bold text-neutral-400">{instance.version || '0.0.0'}</span>
					{#if isOutdated}
			...
			<div class="flex flex-col hidden lg:flex ml-6">
				<span class="text-[8px] font-jetbrains font-black text-neutral-700 uppercase tracking-widest">Players</span>
				<div class="flex items-center gap-3">
					<div class="w-20 h-1 bg-black border border-neutral-800 rounded-none overflow-hidden relative p-0">
					...
					<span class="text-[10px] font-jetbrains font-black text-neutral-400 tracking-tighter">{instance.player_count || 0}</span>
				</div>
			</div>
		</button>
		...
		>
			...
			{#if instance.status !== 'Running'}
				<Button
					...
					icon="ph:play-bold"
					title="Start"
					class="!text-emerald-600 hover:!text-emerald-500 hover:bg-emerald-500/5"
				/>
			{:else}
				<Button
					...
					icon="ph:stop-bold"
					title="Stop"
					class="hover:!text-amber-500 hover:bg-amber-500/5"
				/>
				<Button
	...
	{#if expanded}
		<div
			...
			class="bg-neutral-950/60 border-t border-neutral-800 p-8 space-y-8 relative z-10"
		>
			<div class="grid grid-cols-1 xl:grid-cols-12 gap-10">
				<!-- Left: Technical Readouts -->
				<div class="xl:col-span-7 space-y-8">
					<!-- Primary Toolbar -->
					<div class="flex flex-wrap gap-3 pb-8 border-b border-neutral-800">
						<Button
							onclick={() => dispatch('tail', { nodeId, instanceId: instance.id })}
							variant="secondary"
							size="xs"
						...
						<!-- Version Controls -->
						<div class="flex gap-1 bg-black/40 p-1 border border-neutral-800">
							<Button
					...
					<div class="space-y-4">
						<div class="flex justify-between items-end">
							<div class="flex flex-col gap-1">
								<span class="text-[9px] font-jetbrains font-black text-neutral-600 uppercase tracking-[0.2em]">Usage Telemetry</span>
								<h4 class="text-[11px] font-heading font-black text-neutral-300 uppercase tracking-[0.1em]">Concurrent Players (24h)</h4>
							</div>
						...
						<div class="bg-black/60 border border-neutral-800 p-6 industrial-sharp shadow-inner">
				...
				<div class="xl:col-span-5 space-y-6">
					<div class="bg-neutral-900/40 border border-neutral-800 p-6 space-y-6 industrial-sharp">
						<div class="space-y-3">
							<label for={'name-' + instance.id} class="text-[10px] font-jetbrains font-black text-neutral-600 uppercase tracking-[0.2em] block">Label</label>
							<div class="flex gap-3">
								<input
									id={'name-' + instance.id}
									type="text"
									bind:value={renameValue}
									class="flex-1 bg-neutral-950 border border-neutral-800 px-4 py-2.5 text-xs font-jetbrains font-bold text-white focus:border-rust outline-none transition-all uppercase tracking-widest"
						...
						<div class="grid grid-cols-2 gap-4 pt-2">
							<div class="bg-neutral-950 border border-neutral-800 p-4 flex flex-col gap-1 shadow-inner">
								<span class="text-[9px] font-jetbrains font-black text-neutral-600 uppercase tracking-widest">Process ID</span>
								<span class="text-xs font-jetbrains font-black text-neutral-300 uppercase tracking-tighter">{instance.pid || 'N/A'}</span>
							</div>
							<div class="bg-neutral-950 border border-neutral-800 p-4 flex flex-col gap-1 shadow-inner">
								<span class="text-[9px] font-jetbrains font-black text-neutral-600 uppercase tracking-widest">Network Port</span>
					...
					<div class="bg-amber-500/5 border border-amber-500/20 p-5 flex gap-5 items-start industrial-sharp">
						...
						<div class="space-y-2">
							...
							<p class="text-[11px] font-jetbrains font-medium text-neutral-500 leading-relaxed uppercase tracking-tight">Standard health checks are active. Manual intervention is only required if heartbeats fail to sync.</p>
						</div>
					</div>
				</div>
			</div>
...
<style>
	...
	.heartbeat-pulse::before {
		...
		position: absolute;
		inset: 0;
		background: radial-gradient(circle at 0% 50%, rgba(194, 65, 12, 0.05) 0%, transparent 50%);
		pointer-events: none;
