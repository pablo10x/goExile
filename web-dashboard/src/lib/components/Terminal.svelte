<script lang="ts">
    ...
    function colorize(line: string) {
        if (line.includes('Error') || line.includes('Exception') || line.includes('Failed')) return 'text-red-400';
...
<div class="flex flex-col h-full bg-[#0d1117] rounded-lg border border-slate-800 overflow-hidden font-mono text-xs shadow-inner">
    <!-- Terminal Header -->
    <div class="px-4 py-2 bg-slate-900 border-b border-slate-800 flex justify-between items-center select-none">
        <div class="flex items-center gap-2">
            ...
            <span class="text-slate-500 ml-2 font-semibold tracking-wide text-[10px] uppercase">{title}</span>
        </div>
        {#if !autoScroll}
            <button 
