...
<div class="h-full flex flex-col bg-slate-900">
	<!-- Header -->
	<div
		class="p-6 border-b border-slate-800 bg-gradient-to-r from-slate-950 via-slate-900 to-slate-950"
	>
		<div class="flex justify-between items-start">
			...
			<button
				...
				class="px-5 py-2.5 bg-gradient-to-r from-violet-600 to-purple-600 hover:from-violet-500 hover:to-purple-500 text-white rounded-xl font-bold flex items-center gap-2 shadow-lg shadow-violet-900/30 transition-all transform hover:-translate-y-0.5 active:scale-95"
			>
				<Plus class="w-5 h-5" />
				New Function
			</button>
		...
		<div class="flex gap-4 mt-6">
			<div class="relative flex-1 max-w-md">
				...
				<input
					...
					class="w-full pl-10 pr-4 py-2.5 bg-slate-800/50 border border-slate-700/50 rounded-xl text-slate-200 placeholder-slate-500 focus:border-violet-500/50 focus:ring-2 focus:ring-violet-500/20 outline-none transition-all"
				/>
			</div>
			...
			<select
				...
				class="px-4 py-2.5 bg-slate-800/50 border border-slate-700/50 rounded-xl text-slate-200 focus:border-violet-500/50 outline-none cursor-pointer"
			>
			...
			<button
				...
				class="p-2.5 bg-slate-800/50 border border-slate-700/50 rounded-xl text-slate-400 hover:text-violet-400 hover:border-violet-500/50 transition-all"
				title="Refresh"
			>
		...
		{:else}
			<div class="space-y-3">
				{#each filteredFunctions as fn (fn.oid)}
					<div
						class="bg-slate-800/50 border border-slate-700/50 rounded-xl overflow-hidden hover:border-slate-600/50 transition-all group"
					...
					>
							...
							<div class="flex items-start justify-between">
								<div class="flex items-start gap-3 flex-1 min-w-0">
									<button
										...
										class="p-1 mt-0.5 text-slate-500 hover:text-slate-300 transition-colors"
									>
									...
									<div class="flex-1 min-w-0">
										<div class="flex items-center gap-3 flex-wrap">
											...
											<span
												class="px-2 py-0.5 text-xs font-mono rounded-full bg-slate-700/50 text-slate-400 border border-slate-600/50"
											>
												{fn.language}
											</span>
										</div>
								...
								>
									...
									<button
										...
										class="p-2 text-slate-400 hover:text-blue-400 hover:bg-blue-500/10 rounded-lg transition-all"
										title="Edit"
									>
										<Edit3 class="w-4 h-4" />
									</button>
						...
						{#if expandedFunctions.has(fn.oid)}
							<div
								class="border-t border-slate-700/50 bg-slate-950/50"
								...
								<div class="p-4">
									<div class="flex items-center justify-between mb-3">
										...
										<button
											...
											class="flex items-center gap-1.5 px-2 py-1 text-xs text-slate-400 hover:text-violet-400 hover:bg-violet-500/10 rounded transition-all"
										>
											{#if copied}
									...
									<pre
										class="p-4 bg-slate-900 rounded-lg border border-slate-800 text-sm text-slate-300 font-mono overflow-x-auto max-h-96"><code
											>{fn.source}</code
										></pre>
...
{#if isCreating || isEditing}
	...
	>
		...
		<div
			class="relative w-full max-w-4xl max-h-[90vh] bg-slate-900 border border-slate-700/50 rounded-2xl shadow-2xl overflow-hidden flex flex-col"
		>
			<!-- Modal Header -->
			<div
				class="p-6 border-b border-slate-800 bg-gradient-to-r from-violet-500/10 via-transparent to-purple-500/10"
			>
				<div class="flex items-center justify-between">
					...
					<button
						onclick={closeModal}
						class="p-2 text-slate-400 hover:text-slate-200 hover:bg-slate-800 rounded-lg transition-all"
					>
						<X class="w-5 h-5" />
					</button>
				</div>
			...
			<div class="flex-1 overflow-y-auto p-6 space-y-6">
				...
				<div class="grid grid-cols-3 gap-4">
					...
					<div class="flex flex-col justify-end gap-2">
						<label
							class="flex items-center gap-2 p-2 rounded-lg bg-slate-800 border border-slate-700 cursor-pointer hover:border-violet-500/50 transition-colors"
						>
							<input
								type="checkbox"
							...
							/>
							<span class="text-sm text-slate-300">Strict</span>
						</label>
						<label
							class="flex items-center gap-2 p-2 rounded-lg bg-slate-800 border border-slate-700 cursor-pointer hover:border-violet-500/50 transition-colors"
						>
							<input
								...
								class="rounded text-violet-500 focus:ring-violet-500 bg-slate-700 border-slate-600"
							/>
							<span class="text-sm text-slate-300">Security Definer</span>
						</label>
					</div>
				</div>
...
END;"
			...
			<div class="p-6 border-t border-slate-800 bg-slate-950/50 flex justify-end gap-3">
				<button
					onclick={closeModal}
					class="px-5 py-2.5 text-sm font-semibold text-slate-400 hover:text-slate-200 hover:bg-slate-800 rounded-xl transition-all"
				>
					Cancel
				</button>
				<button
					...
					disabled={loading}
					class="px-6 py-2.5 bg-gradient-to-r from-violet-600 to-purple-600 hover:from-violet-500 hover:to-purple-500 text-white rounded-xl font-bold shadow-lg disabled:opacity-50 transition-all flex items-center gap-2"
				>
					{#if loading}
...
{#if executeModalOpen && functionToExecute}
	...
	>
		...
		<div
			class="relative w-full max-w-2xl max-h-[80vh] bg-slate-900 border border-slate-700/50 rounded-2xl shadow-2xl overflow-hidden flex flex-col"
		>
			<!-- Modal Header -->
			<div
				class="p-6 border-b border-slate-800 bg-gradient-to-r from-emerald-500/10 via-transparent to-cyan-500/10"
			>
				<div class="flex items-center justify-between">
					<div class="flex items-center gap-3">
									...
									<thead class="bg-slate-900 text-slate-400">
										<tr>
											{#each Object.keys(executeResult[0]) as key}
												<th class="px-4 py-2 text-left font-medium border-b border-slate-800"
													>{key}</th
												>
											{/each}
										</tr>
										...
										{#each executeResult as row}
											<tr class="hover:bg-slate-800/50">
												{#each Object.values(row) as val}
													<td class="px-4 py-2 text-slate-300 font-mono">
			...
			<div class="p-6 border-t border-slate-800 bg-slate-950/50 flex justify-end gap-3">
				...
				<button
					...
					class="px-6 py-2.5 bg-gradient-to-r from-emerald-600 to-cyan-600 hover:from-emerald-500 hover:to-cyan-500 text-white rounded-xl font-bold shadow-lg disabled:opacity-50 transition-all flex items-center gap-2"
				>
					{#if executeLoading}
						<RefreshCw class="w-4 h-4 animate-spin" />
						Executing...
