<script lang="ts">
    ...
    async function restoreBackup(filename: string) {
        if (!confirm(`WARNING: This will overwrite the current database with '${filename}'. Are you sure?`)) return;
        loading = true;
        try {
                ...
                {#each backups as backup}
                    <div class="flex items-center justify-between p-4 bg-slate-800/50 border border-slate-700/50 rounded-xl hover:bg-slate-800 hover:border-slate-600 transition-all group">
                        ...
                        <div class="flex gap-2">
                            ...
                            <button onclick={() => deleteBackup(backup.name)} class="p-2 text-slate-500 hover:text-red-400 hover:bg-red-500/10 rounded-lg" title="Delete">
