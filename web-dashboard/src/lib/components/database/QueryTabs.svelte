<script lang="ts">
	...
	let { tabs = [], activeTabId, onSelect, onClose } = $props<{
		tabs: { id: string; label: string; type: 'table' | 'sql' | 'info' | 'config' | 'roles' | 'backups' | 'browser'; icon?: any }[];
...
<div class="flex items-center w-full bg-slate-950 border-b border-slate-800 overflow-x-auto no-scrollbar">
