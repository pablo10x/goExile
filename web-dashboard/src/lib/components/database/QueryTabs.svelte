...
<div
	class="flex items-center w-full bg-[var(--header-bg)] border-b border-stone-800 overflow-x-auto no-scrollbar"
>
	{#each tabs as tab (tab.id)}
		<div
			...
			tab.id
				? 'bg-[var(--terminal-bg)] text-rust border-t-2 border-t-rust'
				: 'text-text-dim hover:bg-[var(--terminal-bg)]/50 hover:text-stone-300 border-t-2 border-t-transparent'}"
		...
		>
			...
			{:else if tab.type === 'sql'}
				<FileCode class="w-3.5 h-3.5 shrink-0 text-warning" />
			{:else}
			...
			<button
				...
				class="p-1 rounded-none opacity-0 group-hover:opacity-100 hover:bg-stone-800 text-text-dim hover:text-danger transition-all"
			>
				<X class="w-3 h-3" />
			</button>
		</div>
