<script lang="ts">
    ...
    let filteredConfig = $derived(
        config.filter(c => c.name.toLowerCase().includes(filter.toLowerCase()) || (c.description && c.description.toLowerCase().includes(filter.toLowerCase())))
    );
...
<div class="h-full flex flex-col bg-slate-900">
        ...
        <div class="flex gap-2">
            {#if Object.keys(pendingChanges).length > 0}
                <button 
                    onclick={saveChanges} 
                    disabled={loading}
                    class="px-4 py-2 bg-orange-600 hover:bg-orange-500 text-white rounded-lg font-bold flex items-center gap-2 shadow-lg animate-pulse"
                >
    ...
    <!-- Toolbar -->
    <div class="px-6 py-3 bg-slate-900/50 border-b border-slate-800 flex justify-between">
        <div class="relative w-64">
            ...
            <input 
                ...
                class="w-full bg-slate-950 border border-slate-700 rounded-lg pl-9 pr-3 py-2 text-sm text-slate-200 focus:border-purple-500 outline-none"
            />
        </div>
    ...
    <div class="flex-1 overflow-auto">
        ...
        {:else}
            <table class="w-full text-left text-sm border-collapse">
                <thead class="bg-slate-950 text-slate-400 sticky top-0 shadow-sm z-10">
                    <tr>
                        <th class="px-6 py-3 border-b border-slate-800 w-1/3">Parameter</th>
                        <th class="px-6 py-3 border-b border-slate-800 w-1/3">Value</th>
                        <th class="px-6 py-3 border-b border-slate-800">Description</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-800/50">
                    {#each filteredConfig as cfg}
                        <tr class="hover:bg-slate-800/30 transition-colors {pendingChanges[cfg.name] ? 'bg-orange-500/5' : ''}">
                            ...
                            <td class="px-6 py-3">
                                <input 
                                    type="text" 
                                    value={pendingChanges[cfg.name] !== undefined ? pendingChanges[cfg.name] : cfg.setting}
                                    oninput={(e) => updateValue(cfg.name, e.currentTarget.value)}
                                    class="w-full bg-slate-900 border border-slate-700 rounded px-2 py-1 text-slate-200 focus:border-orange-500 outline-none font-mono text-xs"
                                />
                            </td>
