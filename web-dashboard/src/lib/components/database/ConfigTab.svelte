...
<div class="h-full flex flex-col bg-transparent">
	...
	>
		<div class="flex items-center gap-4">
			...
			<div>
				...
				<p class="font-jetbrains text-[10px] text-neutral-500 uppercase tracking-widest mt-1 italic font-bold">Tune database parameters (postgresql.conf)</p>
			</div>
		</div>
	...
	<!-- Toolbar -->
	<div
		class="px-6 py-4 bg-neutral-900/20 border-b border-neutral-800 flex justify-between items-center"
	>
		<div class="relative w-80">
			<Search class="absolute left-3 top-1/2 -tranneutral-y-1/2 w-4 h-4 text-neutral-600" />
			<input
				type="text"
				bind:value={filter}
				placeholder="FILTER_PARAMS..."
				class="w-full bg-neutral-950/40 border border-neutral-800 py-2.5 pl-10 pr-4 font-jetbrains text-[10px] text-neutral-300 placeholder:text-neutral-700 focus:border-indigo-500 outline-none uppercase tracking-widest rounded-xl"
			/>
		</div>
	...
	<div class="flex-1 overflow-auto custom-scrollbar bg-transparent">
		...
		{:else}
			<table class="w-full text-left font-jetbrains text-[11px] border-collapse">
				<thead
					class="bg-neutral-950/50 text-neutral-500 sticky top-0 z-10 border-b border-neutral-800 backdrop-blur-md"
				>
					<tr>
						<th class="px-6 py-4 font-bold uppercase tracking-widest w-1/3 border-r border-neutral-800/30 italic"
							>Parameter_Key</th
						>
						<th class="px-6 py-4 font-bold uppercase tracking-widest w-1/4 border-r border-neutral-800/30 italic">Target_Value</th>
				...
				<tbody class="divide-y divide-neutral-800/50">
					{#each filteredConfig as cfg}
						...
						>
							<td class="px-6 py-4 font-bold text-indigo-400/80 uppercase tracking-tighter border-r border-neutral-800/20 italic">
								{cfg.name}
							</td>
							<td class="px-6 py-4 border-r border-neutral-800/20">
								<input
									type="text"
									value={isModified ? pendingChanges[cfg.name] : cfg.setting}
									oninput={(e) => updateValue(cfg.name, e.currentTarget.value)}
									class="w-full bg-neutral-950/40 border {isModified ? 'border-indigo-500' : 'border-neutral-800'} py-1.5 px-3 text-neutral-200 font-jetbrains text-[10px] focus:border-indigo-500 outline-none rounded-lg"
								/>
							</td>
							<td class="px-6 py-4 text-neutral-500 font-medium uppercase tracking-tight leading-relaxed text-[10px]">
								{cfg.description}
							</td>
						</tr>
					{/each}
