<script lang="ts">
	...
	function getBgClass(type: string) {
		switch (type) {
			...
			default:
				return 'bg-neutral-800/10';
		}
	}
</script>

<div
	class="fixed top-6 right-6 left-6 sm:left-auto z-[10000] flex flex-col gap-3 pointer-events-none w-auto sm:w-96"
>
			...
			<div class="p-4 pl-5 flex items-start gap-4">
				...
				<div class="flex-1 min-w-0">
					<div class="flex items-center justify-between gap-2 mb-1">
						<span
							class="text-[10px] font-black font-jetbrains uppercase tracking-widest {colorClass}"
						>
							SYS_{n.type === 'info' ? 'MSG' : n.type.toUpperCase()}
						</span>
						<span class="text-[8px] font-mono text-stone-600"
							>0x{n.id.slice(0, 4).toUpperCase()}</span
						>
					</div>
					...
					{#if n.details}
						<p
							class="mt-1.5 text-[10px] font-mono text-stone-500 break-all border-l border-stone-800 pl-2"
						>
							{n.details}
						</p>
					{/if}
