...
{#if isOpen}
	<div class="fixed inset-0 z-[150] flex justify-end">
		...
		<!-- Drawer Content -->
		<div
			class="relative w-full max-w-2xl bg-[#050505] border-l border-stone-800 shadow-2xl h-full flex flex-col transform transition-transform duration-300 ease-out glass-panel"
		...
		>
			...
			<!-- Header -->
			<div
				class="flex items-center justify-between px-8 py-6 border-b border-stone-800 bg-[#0a0a0a] relative z-20"
			>
				...
				<button
					onclick={onClose}
					class="p-2 text-stone-500 hover:text-white hover:bg-red-950/30 hover:border-red-500/30 border border-transparent transition-all rounded-none group"
				>
			...
			<!-- Footer Decoration -->
			<div class="h-2 bg-stone-900 border-t border-stone-800 flex items-center px-4 gap-2">
