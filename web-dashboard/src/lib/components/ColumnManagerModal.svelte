<script lang="ts">
	...
	let { isOpen = $bindable(false), schema, table, onClose, onSave } = $props<{
		isOpen: boolean;
		schema: string;
		table: string;
		onClose: () => void;
...
{#if isOpen}
	...
	>
		<div class="bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-xl w-full max-w-md shadow-2xl flex flex-col">
			<div class="p-4 border-b border-slate-300 dark:border-slate-700 flex justify-between items-center">
				<h3 class="text-lg font-bold text-slate-100">Add Column to <span class="text-blue-400 font-mono">{table}</span></h3>
			...
			<div class="p-6 space-y-4">
				<div>
					<label for="column-name" class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">Column Name</label>
					<input
						id="column-name"
						type="text"
				...
				<div>
					<label for="column-type" class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">Data Type</label>
					<select
						id="column-type"
