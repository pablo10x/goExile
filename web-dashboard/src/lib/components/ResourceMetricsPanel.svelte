...
<div class="space-y-6">
		...
		<div class="flex items-center justify-between">
			<div>
				<h3 class="text-lg font-bold text-slate-100 flex items-center gap-2">
					...
					{#if $loading}
						<div
							class="w-3 h-3 rounded-full border-2 border-slate-500 border-t-blue-500 animate-spin"
						></div>
					{/if}
				</h3>
			...
			<div class="flex items-center gap-2">
				<div class="flex bg-slate-900/50 rounded-lg p-0.5 border border-slate-800">
					{#each timeRanges as range}
						<button
							class="px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider rounded transition-all {timeRange ===
							range
	...
	{#if $error}
		<div class="bg-red-500/10 border border-red-500/20 rounded-lg p-4 flex items-start gap-3">
			...
			<div class="text-sm text-red-300">
				<span class="font-bold">Error fetching metrics:</span>
				{$error}
			</div>
		</div>
	{/if}
	...
	{#if !$error}
		...
		{#if !compact}
			<div
				class="bg-slate-900/30 rounded-xl border border-slate-800 p-1 relative overflow-hidden group"
			>
				<div
					class="absolute top-3 left-4 text-xs font-bold text-slate-500 uppercase tracking-widest z-10 pointer-events-none"
				>
