<script lang="ts">
    ...
    import type { TopResourceConsumersProps, ResourceConsumer, ResourceType } from '$lib/types/resource-metrics';
        ...
        {:else}
            <div class="space-y-3">
                {#each consumers as consumer, index (consumer.id)}
                    <div class="group flex items-center justify-between p-3 bg-slate-900/30 rounded-lg border border-slate-700/50 hover:border-slate-600/50 transition-all duration-300">
                        <!-- Instance Info -->
                        ...
                        <div class="flex items-center gap-4">
                            <div class="text-right">
                                <div class="text-lg font-bold tabular-nums {getColorForValue(consumer.cpu_percent, resourceType)}">
                            ...
                            <div class="w-16 h-8 bg-slate-700/50 rounded-full overflow-hidden relative">
                                <div 
                                    class="h-full {getBackgroundForValue(consumer.cpu_percent, resourceType)} transition-all duration-500"
                        ...
                        <!-- Status -->
                        <div class="flex items-center gap-2">
                            <div class="w-2 h-2 rounded-full {consumer.status === 'Running' ? 'bg-emerald-500' : 'bg-slate-600'}"></div>
                            <div class="text-xs text-slate-400">
                                {consumer.status}
                            </div>
                        </div>
    ...
    {#if loading}
        <div class="space-y-3">
            {#each Array(limit) as _, index (index)}
                <div class="flex items-center justify-between p-3 bg-slate-900/30 rounded-lg border border-slate-700/50">
