...
<div class="select-none">
	...
	>
		{#if type !== 'column'}
			<div
				class="w-4 h-4 flex items-center justify-center transition-transform duration-200 {isOpen
					? 'rotate-90'
					: ''}"
			>
		...
		{#if isLoading}
			<div
				class="ml-auto w-3 h-3 rounded-full border-2 border-slate-500 border-t-transparent animate-spin"
			></div>
		{/if}
	</div>
