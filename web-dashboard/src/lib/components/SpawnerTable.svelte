<script lang="ts">
	...
	function dispatchBulkAction(action: 'start' | 'stop' | 'restart' | 'update', spawnerId: number) {
		...
		console.log(
			`Dispatching bulk action: ${action} for spawner ${spawnerId}. Target instances:`,
			targetInstances.map((i) => i.id)
		);
	...
	<table class="w-full text-sm">
		<thead class="border-b border-slate-700">
			<tr>
				...
				>
				<th
					class="text-left px-4 py-3 text-xs font-semibold text-slate-400 uppercase tracking-wider text-right"
					>Actions</th
				>
			</tr>
		</thead>
		<tbody class="text-slate-300">
			{#each spawners as spawner}
				...
				<tr
					class="border-t border-slate-700 hover:bg-slate-800/50 transition cursor-pointer {spawner.id ===
						...
						<td colspan="7" class="px-4 py-4">
							...
							<div class="space-y-2">
											...
											<div slot="default" let:close>
												...
												>
													<svg
														...
														stroke-linecap="round"
														stroke-linejoin="round"
														><path d="M23 4v6h-6"></path><path d="M1 20v6h6"></path><path
								...
								{:else if !activeInstances[spawner.id] || activeInstances[spawner.id].length === 0}
									<div
										class="p-4 text-center text-slate-500 text-xs bg-slate-800/30 rounded border border-slate-700/50"
									>
