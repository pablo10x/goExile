	...
	>
		<div class="bg-slate-900 border border-slate-700 rounded-xl w-full max-w-2xl shadow-2xl flex flex-col max-h-[90vh]">
			...
			<div class="p-6 overflow-y-auto space-y-6">
				<div>
					<label for="tableName" class="block text-sm font-medium text-slate-300 mb-1.5">Table Name</label>
					<input
						type="text"
						id="tableName"
				...
				<div class="space-y-3">
					<div class="flex justify-between items-center">
						<h4 class="text-sm font-bold text-slate-400 uppercase tracking-wider">Columns</h4>
						<button
							onclick={addColumn}
					...
					<div class="space-y-2">
						{#each columns as col, i}
							<div class="flex flex-col gap-2 p-3 bg-slate-950/50 border border-slate-800 rounded-lg group">
								<div class="flex gap-2 items-start">
							<div class="flex flex-col gap-1 w-full">
								<label for="columnName-{i}" class="text-xs text-slate-400">Name</label>
								<input
									type="text"
									id="columnName-{i}"
									bind:value={col.name}
									placeholder="Column Name"
									class="flex-1 bg-slate-900 border border-slate-700 rounded px-2 py-1.5 text-sm text-slate-200 outline-none focus:border-blue-500"
								/>
							</div>
							<div class="flex flex-col gap-1 w-full">
								<label for="columnType-{i}" class="text-xs text-slate-400">Type</label>
								<select
									id="columnType-{i}"
									bind:value={col.type}
									class="bg-slate-900 border border-slate-700 rounded px-2 py-1.5 text-sm text-blue-400 outline-none focus:border-blue-500 font-mono"
								>
			...
			<div class="p-4 border-t border-slate-700 bg-slate-900/50 flex justify-end gap-3">
				...
				<button
					...
					class="px-6 py-2 bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg font-bold flex items-center gap-2 shadow-lg disabled:opacity-50"
				>
					{#if loading}
