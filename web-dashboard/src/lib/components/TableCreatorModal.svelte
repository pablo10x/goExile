<script lang="ts">
	...
	let { isOpen = $bindable(false), schema = 'public', onClose, onSave } = $props<{
		isOpen: boolean;
		schema: string;
		onClose: () => void;
...
{#if isOpen}
	...
	>
		<div class="bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-xl w-full max-w-2xl shadow-2xl flex flex-col max-h-[90vh]">
			<div class="p-4 border-b border-slate-300 dark:border-slate-700 flex justify-between items-center">
				<h3 class="text-lg font-bold text-slate-100">Create Table in <span class="text-blue-400 font-mono">{schema}</span></h3>
				<button onclick={onClose} class="text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:text-white">
					<X class="w-5 h-5" />
				</button>
			</div>
			...
			<div class="p-6 overflow-y-auto space-y-6">
				<div>
					<label for="tableName" class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">Table Name</label>
					<input
						type="text"
						id="tableName"
				...
				<div class="space-y-3">
					<div class="flex justify-between items-center">
						<h4 class="text-sm font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Columns</h4>
						<button
							onclick={addColumn}
					...
					<div class="space-y-2">
						{#each columns as col, i}
							<div class="flex flex-col gap-2 p-3 bg-white/50 dark:bg-slate-950/50 border border-slate-200 dark:border-slate-800 rounded-lg group">
								<div class="flex gap-2 items-start">
							<div class="flex flex-col gap-1 w-full">
								<label for="columnName-{i}" class="text-xs text-slate-500 dark:text-slate-400">Name</label>
								<input
									type="text"
									id="columnName-{i}"
									bind:value={col.name}
							...
							<div class="flex flex-col gap-1 w-full">
								<label for="columnType-{i}" class="text-xs text-slate-500 dark:text-slate-400">Type</label>
								<select
									id="columnType-{i}"
									bind:value={col.type}
								...
								<div class="flex flex-wrap gap-2">
									{#each constraints as c}
										<button
											...
											class="px-2 py-0.5 rounded text-xs border transition-colors {col.constraints.includes(c)
			...
			<div class="p-4 border-t border-slate-300 dark:border-slate-700 bg-slate-900/50 flex justify-end gap-3">
				<button
					onclick={onClose}
