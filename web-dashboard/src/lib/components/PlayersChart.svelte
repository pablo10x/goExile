    ...
    {#if width > 0 && data.length > 1}
        <svg {width} {height} class="overflow-visible">
            ...
            {#if hoveredIndex !== null}
                ...
                {@const y = height - ((d.count - minVal) / (maxVal - minVal)) * (height * 0.7) - (height * 0.15)}
                
                <!-- Vertical Cursor Line -->
                <line x1={x} y1={0} x2={x} y2={height} stroke="white" stroke-opacity="0.1" stroke-width="1" />
                
                <circle cx={x} cy={y} r="4" fill={color} stroke="white" stroke-width="2" class="pointer-events-none" />
            {/if}
        </svg>
        ...
        {#if hoveredIndex !== null}
            ...
            >
                <div class="text-xs text-slate-400 font-mono mb-0.5">{formatTime(d.timestamp)}</div>
