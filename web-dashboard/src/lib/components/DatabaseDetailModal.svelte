        ...
        <div class="space-y-4">
            ...
            <div class="grid grid-cols-2 gap-3">
                <div class="bg-slate-800/50 p-3 rounded-lg border border-slate-700/30">
                    ...
                    <div class="text-lg font-bold text-slate-100 font-mono">{stats.db_open_connections} <span class="text-slate-500 text-sm">/ 10</span></div>
                </div>
                <div class="bg-slate-800/50 p-3 rounded-lg border border-slate-700/30">
                    <div class="text-xs text-slate-400 mb-1">In Use / Idle</div>
                    <div class="text-lg font-bold text-emerald-400 font-mono">{stats.db_in_use} <span class="text-slate-500 text-sm">/ {stats.db_idle}</span></div>
                </div>
            </div>

            <!-- Wait Stats -->
             <div class="bg-slate-800/50 p-3 rounded-lg border border-slate-700/30 flex items-center justify-between">
                <div>
            ...
            <div class="grid grid-cols-2 gap-x-4 gap-y-3 text-sm">
                <div class="flex justify-between items-center">
                    <span class="text-slate-400 flex items-center gap-1.5"><HardDrive class="w-3 h-3"/> DB Size</span>
                    <span class="font-mono text-slate-200">{stats.db_size || 'N/A'}</span>
                </div>
                <div class="flex justify-between items-center">
                    <span class="text-slate-400 flex items-center gap-1.5"><Activity class="w-3 h-3"/> Cache Hit</span>
                    <span class="font-mono text-emerald-400">{stats.db_cache_hit ? stats.db_cache_hit.toFixed(2) : 0}%</span>
                </div>
                 <div class="flex justify-between items-center">
                    <span class="text-slate-400 flex items-center gap-1.5"><Server class="w-3 h-3"/> Commits</span>
                    <span class="font-mono text-slate-200">{stats.db_commits || 0}</span>
                </div>
                 <div class="flex justify-between items-center">
                    <span class="text-slate-400 flex items-center gap-1.5"><Archive class="w-3 h-3"/> Rollbacks</span>
                    <span class="font-mono text-red-400">{stats.db_rollbacks || 0}</span>
                </div>
            </div>
            
            <!-- Tuple Stats -->
            <div class="bg-slate-800/30 p-2 rounded border border-slate-700/30 grid grid-cols-4 gap-1 text-center text-xs mt-2">
                <div>
