<script lang="ts">
	...
	import { notifications, siteSettings, backgroundConfig, theme } from '$lib/stores.svelte';
	...
	let filteredFirebaseConfigs = $derived.by(() => {
		...
		return firebaseConfigs.filter(
			(c) => c.key.toLowerCase().includes(query) || c.description.toLowerCase().includes(query)
		);
	});
	...
	async function loadConfig() {
		...
		try {
			...
			notifications.add({ type: 'error', message: 'Config Error', details: e.message });
		} finally {
			loading = false;
		}
	}
	...
	async function deleteFirebaseParameter(key: string) {
		...
		try {
			...
			if (res.ok) {
				notifications.add({ type: 'success', message: 'Deleted' });
				await loadFirebaseStatus();
			}
		} catch (e: any) {
	...
	async function syncFirebaseConfig() {
		...
		try {
			...
			if (res.ok) {
				notifications.add({ type: 'success', message: 'Sync complete' });
				await loadFirebaseStatus();
			}
		} catch (e: any) {
			notifications.add({ type: 'error', message: 'Sync failed', details: e.message });
		} finally {
			loading = false;
		}
	}
	...
	async function saveChanges() {
		...
		try {
			...
			if (rs.every((r) => r.ok)) {
				notifications.add({ type: 'success', message: 'Saved' });
				pendingChanges = new Map();
				await loadConfig();
			} else throw new Error('Some items failed to save');
		} catch (e: any) {
			notifications.add({ type: 'error', message: 'Error', details: e.message });
		} finally {
			saving = false;
		}
	}
...
<div class="space-y-10 font-sans">
	<PageHeader
		...
		subtitle="Global environment variables & parameters"
		icon="ph:sliders-bold"
	>
		{#snippet actions()}
			<div class="flex items-center gap-4">
				{#if hasUnsavedChanges}
					<div
						class="flex items-center gap-3 px-4 py-2 bg-sky-500/10 border border-sky-500/20 rounded-xl"
						transition:fade
					>
						...
						<span class="text-xs font-bold text-sky-400 uppercase tracking-wider"
							>{pendingChangeCount} Pending Changes</span
						>
					</div>
					<Button
						onclick={() => {
						...
						variant="secondary"
						size="md">Discard</Button
					>
					<Button onclick={saveChanges} loading={saving} variant="primary" size="md"
				...
				{:else}
					<Button
						...
						variant="secondary"
						size="md"
						icon="ph:arrows-clockwise-bold">Reload</Button
					>
				{/if}
			</div>
		{/snippet}
	...
	<div class="flex bg-black/20 p-1.5 rounded-2xl border border-white/5 backdrop-blur-md">
		{#each [{ id: 'system', label: 'Core System', sub: 'Global Settings' }, { id: 'nodes', label: 'Infrastructure', sub: 'Node Defaults' }, { id: 'firebase', label: 'Remote Config', sub: 'Firebase API' }] as tab}
			...
			>
				<span class="text-[13px] font-bold uppercase tracking-tight {activeTab === tab.id ? 'text-white' : ''}">{tab.label}</span>
				<span class="text-[9px] font-bold uppercase tracking-[0.15em] opacity-40 {activeTab === tab.id ? 'text-sky-400/60' : ''}">{tab.sub}</span>
			</button>
		{/each}
	</div>
	...
	<div class="relative group">
		<Search
			class="absolute left-5 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-500 group-focus-within:text-sky-400 transition-colors"
		/>
		<input
			type="text"
			bind:value={searchQuery}
			placeholder="Search settings..."
			class="w-full pl-14 pr-6 py-4 bg-slate-900/50 border border-white/5 rounded-2xl text-white text-sm focus:border-sky-500/50 outline-none transition-all shadow-inner backdrop-blur-sm"
		/>
	</div>
				...
				<div class="space-y-8">
					{#each activeTab === 'system' ? filteredMasterSections : filteredNodeSections as section (section.id)}
						...
						>
							{#snippet actions()}
								<span
									class="text-[10px] font-bold text-slate-500 uppercase tracking-widest bg-white/5 px-3 py-1 rounded-lg"
									>{section.items.length} Settings</span
								>
							{/snippet}
								...
								{#each section.items as item (item.key)}
									...
									<div
										class="p-5 bg-slate-900/40 rounded-2xl border {isPending
											...
											: 'border-white/5'} transition-all group"
									>
											...
											<div class="flex-1">
												<div class="flex items-center gap-3 mb-2 flex-wrap">
													...
													{#if item.requires_restart}<span
															class="text-[9px] font-bold bg-amber-500/10 text-amber-500 px-2 py-0.5 rounded uppercase border border-amber-500/20"
												...
												<div class="mt-4 flex flex-col md:flex-row md:items-center gap-4">
													{#if item.type === 'bool'}
														<button
															...
															class="flex items-center gap-3 px-4 py-2 rounded-xl border transition-all {(pendingChanges.get(
																item.key
															) ?? item.value) === 'true'
														...
														>
															<div
																class="w-2 h-2 rounded-full {(pendingChanges.get(item.key) ??
																	item.value) === 'true'
														...
														<div class="flex items-center gap-2 flex-1 max-w-xl">
															<div class="relative flex-1">
																<input
																	...
																	disabled={item.is_read_only}
																	class="w-full bg-slate-950 border border-white/5 rounded-xl text-white font-mono text-xs px-4 py-2.5 focus:border-sky-500/50 outline-none transition-all disabled:opacity-30"
																/>
																{#if isSecret}<button
																		onclick={() => {
																			if (showSecrets.has(item.key)) showSecrets.delete(item.key);
																			else showSecrets.add(item.key);
																			showSecrets = new Set(showSecrets);
																		}}
																		class="absolute right-3 top-1/2 -translate-y-1/2 text-slate-500 hover:text-sky-400"
																		>{#if showSecrets.has(item.key)}<EyeOff size={16} />{:else}<Eye
																				size={16}
																			/>{/if}</button
																	>{/if}
															</div>
															<button
																onclick={() => {
																	navigator.clipboard.writeText(
																		pendingChanges.get(item.key) ?? item.value
																	);
																	notifications.add({ type: 'success', message: 'Copied' });
																}}
																class="p-2.5 bg-slate-800 text-slate-400 hover:text-white hover:bg-slate-700 rounded-xl border border-white/5 transition-all"
																><Copy size={16} /></button
															>
														</div>
													{/if}
			...
			{#if activeTab === 'firebase'}
				<div class="space-y-6">
					<Card
						...
						subtitle="Synchronize remote client parameters"
						icon="ph:flame-bold"
					>
						{#snippet actions()}
							{#if firebaseConnected}<div
									class="flex items-center gap-2 px-3 py-1 bg-emerald-500/10 border border-emerald-500/20 rounded-lg text-emerald-400 text-[10px] font-bold uppercase"
								>
									<div class="w-1.5 h-1.5 bg-emerald-500 rounded-full animate-pulse"></div>
									Connected
								</div>
							{:else}<div
									class="flex items-center gap-2 px-3 py-1 bg-slate-800 border border-white/5 rounded-lg text-slate-500 text-[10px] font-bold uppercase"
								>
						...
						<div class="p-6">
								...
								<div class="space-y-6">
									<div class="flex items-center justify-between border-b border-white/5 pb-4">
										<div class="flex items-center gap-3">
											<FileJson class="w-5 h-5 text-sky-400" />
											<h3 class="text-sm font-bold text-white uppercase tracking-wider">
												Remote Parameters
											</h3>
										</div>
										<div class="flex gap-2">
											<Button onclick={syncFirebaseConfig} variant="secondary" size="sm"
												>Sync All</Button
											><Button
												onclick={() => openFirebaseModal('create')}
												variant="primary"
												size="sm">Add Parameter</Button
											>
										</div>
									</div>
									<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
										{#each filteredFirebaseConfigs as config (config.key)}
											<div
												class="p-5 bg-slate-900/40 rounded-2xl border border-white/5 hover:border-sky-500/30 transition-all group"
											>
												<div class="flex items-start justify-between gap-4">
													<div class="min-w-0">
														<div class="flex items-center gap-2 mb-1">
															<span class="text-xs font-bold text-sky-400 font-mono uppercase"
																>{config.key}</span
															><span
																class="text-[9px] bg-slate-800 text-slate-500 px-1.5 py-0.5 rounded font-bold"
																>{config.valueType}</span
															>
														</div>
														<p
															class="text-[10px] text-slate-500 font-medium mb-3 truncate uppercase tracking-tight"
														>
															{config.description || 'No description'}
														</p>
														<code
															class="text-[10px] text-slate-400 bg-slate-950 px-3 py-2 rounded-lg block truncate font-mono border border-white/5 shadow-inner"
															>{config.value}</code
														>
													</div>
							...
							{:else}
								<div class="text-center py-12 text-slate-500">
									<CloudCog class="w-12 h-12 mx-auto opacity-20 mb-4" />
...
{#if showFirebaseModal}
	<div
		class="fixed inset-0 z-[500] flex items-center justify-center p-4 bg-slate-950/90 backdrop-blur-md"
		onclick={(e) => e.target === e.currentTarget && (showFirebaseModal = false)}
		role="button"
		tabindex="0"
		onkeydown={null}
		transition:fade
	>
		<div
			class="w-full max-w-lg bg-slate-900 border border-white/10 rounded-3xl shadow-2xl overflow-hidden"
			transition:scale
		>
			<div class="px-8 py-6 border-b border-white/5 flex items-center justify-between">
				<h3 class="text-xl font-bold text-white tracking-tight flex items-center gap-3">
					<Flame class="text-sky-400" />{firebaseModalMode === 'create'
						? 'New Parameter'
						: 'Edit Parameter'}
				</h3>
				<button
					onclick={() => (showFirebaseModal = false)}
					class="p-2 text-slate-500 hover:text-white transition-colors"><X /></button
				>
			</div>
			<div class="p-8 space-y-6 max-h-[70vh] overflow-y-auto no-scrollbar">
				<div>
					...
					><input
						...
						class="w-full bg-slate-950 border border-white/5 rounded-xl px-4 py-3 text-white text-sm focus:border-sky-500/50 outline-none"
					/>
				</div>
				<div>
					<label
					...
					><select
						...
						class="w-full bg-slate-950 border border-white/5 rounded-xl px-4 py-3 text-white text-sm focus:border-sky-500/50 outline-none appearance-none"
						><option value="STRING">STRING</option><option value="NUMBER">NUMBER</option><option
				...
				<div>
					<label
						...
						class="block text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-2 ml-1"
						>Value</label
					>{#if firebaseForm.valueType === 'BOOLEAN'}<select
							id="fbValue"
							bind:value={firebaseForm.value}
							class="w-full bg-slate-950 border border-white/5 rounded-xl px-4 py-3 text-white text-sm focus:border-sky-500/50 outline-none"
							><option value="true">TRUE</option><option value="false">FALSE</option></select
						>{:else if firebaseForm.valueType === 'JSON'}<textarea
							id="fbValue"
							bind:value={firebaseForm.value}
							rows={4}
							placeholder={`{ "active": true }`}
							class="w-full bg-slate-950 border border-white/5 rounded-xl px-4 py-3 text-white text-sm font-mono focus:border-sky-500/50 outline-none resize-none"
						></textarea>{:else}<input
							id="fbValue"
				...
				<div>
					...
					><textarea
						...
						class="w-full bg-slate-950 border border-white/5 rounded-xl px-4 py-3 text-white text-sm focus:border-sky-500/50 outline-none resize-none"
					></textarea>
				</div>
			</div>
			<div
				class="px-8 py-6 bg-slate-950 border-t border-white/5 flex items-center justify-end gap-4"
			>
				<Button onclick={() => (showFirebaseModal = false)} variant="ghost" size="md">Cancel</Button
				><Button
					onclick={saveFirebaseParameter}
					disabled={firebaseSaving || !firebaseForm.key.trim()}
					variant="primary"
					size="md"
					loading={firebaseSaving}>Save Parameter</Button
				>
			</div>
		</div>
	</div>
