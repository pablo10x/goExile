	...
	{:else}
		...
		<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
			{#each categories as category}
				...
				>
					<div
						class="bg-slate-800/60 backdrop-blur-sm border border-slate-700 rounded-xl p-6 hover:border-{category.color}-500/50 transition-all duration-300 group-hover:shadow-lg group-hover:shadow-{category.color}-900/20"
					>
						<div class="flex items-start justify-between mb-4">
							<div class="p-2 bg-{category.color}-500/10 rounded-lg">
								<svg
									xmlns="http://www.w3.org/2000/svg"
									class="w-6 h-6 text-{category.color}-400"
									viewBox="0 0 24 24"
									fill="currentColor"
								>
		...
		{#if $config && $config.length > 0}
			<div
				class="bg-slate-800/60 backdrop-blur-sm border border-slate-700 rounded-xl p-6 transform hover:scale-[1.01] transition-transform duration-300"
			>
				<h2 class="text-xl font-semibold text-slate-100 mb-4">All Configuration Settings</h2>
				<div class="space-y-3">
					{#each $config as configItem (configItem.key)}
						<div
							class="flex items-center justify-between p-3 bg-slate-900/60 rounded-lg border border-slate-700"
						>
							<div class="flex items-center gap-3">
								<div
									class="w-2 h-2 rounded-full bg-{getCategoryColor(configItem.category)}-500"
								></div>
								<div>
