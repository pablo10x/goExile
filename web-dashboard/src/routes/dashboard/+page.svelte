  ...
  onMount(() => {
    ...
    async function executeInstanceAction() {
        ...
        try {
            ...
            notifications.add({ type: 'error', message: `Failed to ${instanceActionType} instance: ${e.message || 'Unknown error'}` });
        }
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
    <div class="lg:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6 h-full">
        ...
        <div 
            class="transform transition-all duration-700 hover:scale-105 hover:-translate-y-1 {animateStats ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'}" 
        ...
        >
            <StatsCard 
                ...
                subValue={$stats.db_connected ? `<span class="text-emerald-400">${$stats.db_open_connections} Open</span> <span class="text-slate-600 mx-2">|</span> <span class="text-blue-400">${$stats.db_in_use} In Use</span>` : ''}
                Icon={Database}
