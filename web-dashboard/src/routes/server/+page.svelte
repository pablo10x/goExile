        ...
        <div class="grid lg:grid-cols-3 gap-6">
                ...
                <div class="bg-slate-800/50 border border-slate-700/50 rounded-xl p-8">
                    <div class="mb-6">
                        ...
                        <p class="text-slate-400 text-sm">
                            Upload a new <code class="px-1 py-0.5 bg-slate-700 rounded text-xs">game_server.zip</code> package with version information and release notes.
                        </p>
                    </div>
                    ...
                    >
                        ...
                        >
                            ...
                            <div class="p-12 text-center">
                                        ...
                                        <div class="space-y-2">
                                            ...
                                            {#if uploadProgress > 0}
                                                <div class="w-full max-w-md mx-auto">
                                                    ...
                                                    <p class="text-xs text-slate-500 mt-1">{Math.round(uploadProgress)}% complete</p>
                                                </div>
                                            {/if}
                                        </div>
                                    </div>
                                {:else if analyzing}
                                    <div class="space-y-4">
                                        <div class="w-16 h-16 mx-auto border-4 border-yellow-500 border-t-transparent rounded-full animate-spin"></div>
                                        <div class="space-y-2">
                                            <p class="text-yellow-400 font-medium">Analyzing file...</p>
                                            <p class="text-xs text-slate-500">Checking compatibility and extracting metadata</p>
                                        </div>
                                    </div>
                                {:else}
                                    <div class="space-y-4">
                                        <div class="w-16 h-16 mx-auto mx-auto flex items-center justify-center rounded-full bg-slate-800 group-hover:bg-slate-700 transition-colors">
                                            <svg class="w-8 h-8 text-slate-400 group-hover:text-slate-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        ...
                                        <div class="space-y-2">
                                            ...
                                            <p class="text-xs text-slate-500">
                                                {selectedFile ? formatFileSize(selectedFile.size) : 'Maximum file size: 1GB • ZIP files only'}
                                            </p>
                                        </div>
                                        {#if !selectedFile}
                                            <button class="px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white rounded-lg text-sm font-medium transition-colors">
                                                Choose File
                                            </button>
                                        {/if}
                                    </div>
                        ...
                        <div class="mt-6 p-4 bg-slate-900/50 rounded-lg border border-slate-700">
                            ...
                            {#if fileAnalysis.compatibility}
                                <div class="mt-3 flex items-center justify-center">
                                    <span class={`px-3 py-1 rounded-full text-xs font-medium border ${getCompatibilityColor(fileAnalysis.compatibility)}`}>
            ...
            <div class="lg:col-span-1 space-y-6">
                ...
                <div class="space-y-4">
                    ...
                    {#if uploadStatus}
                        <div class={`p-4 rounded-lg text-sm font-medium border flex items-start gap-3 ${uploadError ? 'bg-red-500/10 text-red-400 border-red-500/20' : 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20'}`}>
                   ...
                   <div class="p-6 pb-4 relative overflow-hidden">
                                 <div class="flex items-start justify-between mb-3">
                                    <div class="flex items-center gap-3">
                                        ...
                                        {:else}
                                            <span class="inline-flex items-center px-3 py-1.5 rounded-full text-xs font-medium bg-slate-700 text-slate-400">
                                                Inactive
                                            </span>
                                        {/if}
                                    </div>
                                   ...
                                   <!-- Action Buttons that appear on hover -->
                                     <div class="absolute top-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-y-2 group-hover:translate-y-0 z-10">
                                         {#if !version.is_active}
                                             <button 
                                ...
                                <div class="space-y-3">
                                    ...
                                    <div class="space-y-2">
                                        <div class="flex items-center justify-between text-sm">
                                            ...
                                            <span class="text-slate-400">{new Date(version.uploaded_at).toLocaleDateString()}</span>
                                        </div>
                                        <div class="flex items-center justify-between text-sm">
                                            <span class="text-slate-500">Time</span>
                                            <span class="text-slate-400">{new Date(version.uploaded_at).toLocaleTimeString()}</span>
                                        </div>
                                    </div>
