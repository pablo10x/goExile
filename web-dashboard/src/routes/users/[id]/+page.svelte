	...
	<div class="mb-8 flex items-center justify-between">
		<div class="flex items-center gap-4">
			...
			<div>
				...
				<p class="text-slate-500 dark:text-slate-400 mt-1">Detailed view of player profile and social graph</p>
			</div>
		</div>
	...
	{:else if player}
		<div class="grid grid-cols-1 lg:grid-cols-3 gap-6" transition:fade>
			...
			<div class="lg:col-span-2 space-y-6">
				<!-- Identity Card -->
				<div class="bg-slate-800/40 backdrop-blur-sm border border-slate-300/50 dark:border-slate-700/50 rounded-2xl p-6 relative overflow-hidden">
					<div class="absolute top-0 right-0 p-6 opacity-10">
						<User class="w-32 h-32" />
					</div>
					
					<div class="relative z-10 flex flex-col sm:flex-row gap-6 items-start">
						<div class="w-20 h-20 rounded-2xl bg-gradient-to-br from-blue-600 to-indigo-600 flex items-center justify-center text-3xl font-bold text-slate-900 dark:text-white shadow-xl shadow-blue-900/30">
						...
						<div class="flex-1 space-y-4">
							<div>
								<div class="flex items-center gap-3">
									...
									{:else}
										<div class="px-2 py-0.5 bg-slate-700/50 border border-slate-600 text-slate-500 dark:text-slate-400 text-xs font-bold rounded-full">
											OFFLINE
										</div>
									{/if}
								</div>
								<div class="text-slate-500 dark:text-slate-400 font-mono text-sm mt-1">ID: #{player.id}</div>
							</div>

							<div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
								<div class="p-3 bg-slate-900/50 rounded-xl border border-slate-300/30 dark:border-slate-700/30">
									...
									<code class="text-sm text-slate-700 dark:text-slate-300 font-mono break-all">{player.device_id}</code>
								</div>
								<div class="p-3 bg-slate-900/50 rounded-xl border border-slate-300/30 dark:border-slate-700/30">
									<div class="text-xs text-slate-500 uppercase font-bold tracking-wider mb-1 flex items-center gap-2">
										<Users class="w-3 h-3" /> Firebase UID
									</div>
									<code class="text-sm text-slate-700 dark:text-slate-300 font-mono break-all">{player.uid || 'Not Linked'}</code>
								</div>
							</div>
						</div>
					</div>
				...
				<div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
					<div class="bg-slate-800/40 border border-slate-300/50 dark:border-slate-700/50 rounded-xl p-4 flex flex-col items-center justify-center text-center hover:bg-slate-800/60 transition-colors">
						<div class="mb-2 p-2 bg-amber-500/10 rounded-lg text-amber-400">
							<Trophy class="w-5 h-5" />
						</div>
						<div class="text-2xl font-bold text-slate-900 dark:text-white">{player.xp.toLocaleString()}</div>
						<div class="text-xs text-slate-500 dark:text-slate-400 uppercase font-bold tracking-wider">Experience</div>
					</div>
					...
					<div class="bg-slate-800/40 border border-slate-300/50 dark:border-slate-700/50 rounded-xl p-4 flex flex-col items-center justify-center text-center hover:bg-slate-800/60 transition-colors">
						...
						<div class="text-sm font-bold text-slate-900 dark:text-white truncate w-full" title={player.last_joined_server}>
							{player.last_joined_server || 'N/A'}
						</div>
						<div class="text-xs text-slate-500 dark:text-slate-400 uppercase font-bold tracking-wider">Last Server</div>
					</div>
					...
					<div class="bg-slate-800/40 border border-slate-300/50 dark:border-slate-700/50 rounded-xl p-4 flex flex-col items-center justify-center text-center hover:bg-slate-800/60 transition-colors">
						...
						<div class="text-xs text-slate-500 dark:text-slate-400 uppercase font-bold tracking-wider">Joined</div>
					</div>
				</div>

				<!-- Timestamps -->
				<div class="bg-slate-800/40 border border-slate-300/50 dark:border-slate-700/50 rounded-xl p-4 flex items-center justify-between text-sm">
			...
			<div class="space-y-6">
				<!-- Friends List -->
				<div class="bg-slate-800/40 backdrop-blur-sm border border-slate-300/50 dark:border-slate-700/50 rounded-2xl overflow-hidden flex flex-col h-full max-h-[500px]">
					<div class="p-4 border-b border-slate-300/50 dark:border-slate-700/50 bg-slate-900/30 flex items-center justify-between">
						<h3 class="font-bold text-slate-900 dark:text-white flex items-center gap-2">
							<UserCheck class="w-5 h-5 text-emerald-400" />
							Friends
						</h3>
						<span class="px-2 py-0.5 bg-slate-700 rounded-full text-xs font-bold text-slate-700 dark:text-slate-300">
						...
						{#if player.friends && player.friends.length > 0}
							{#each player.friends as friend}
								<a href={`/users/${friend.id}`} class="flex items-center gap-3 p-2 hover:bg-slate-700/30 rounded-xl transition-colors group">
									<div class="w-8 h-8 rounded-lg bg-slate-700 flex items-center justify-center font-bold text-slate-900 dark:text-white text-xs">
										{friend.name.charAt(0).toUpperCase()}
									</div>
									<div class="flex-1 min-w-0">
										<div class="text-sm font-medium text-slate-800 dark:text-slate-200 truncate group-hover:text-blue-400 transition-colors">{friend.name}</div>
				...
				{#if (player.incoming_friend_requests && player.incoming_friend_requests.length > 0) || (player.outgoing_friend_requests && player.outgoing_friend_requests.length > 0)}
					<div class="bg-slate-800/40 backdrop-blur-sm border border-slate-300/50 dark:border-slate-700/50 rounded-2xl overflow-hidden">
						...
						<div class="p-2 space-y-2">
							{#if player.incoming_friend_requests && player.incoming_friend_requests.length > 0}
								...
								{#each player.incoming_friend_requests as req}
									<div class="flex items-center gap-3 p-2 bg-slate-900/30 rounded-xl border border-slate-300/30 dark:border-slate-700/30">
										<div class="w-8 h-8 rounded-lg bg-slate-700 flex items-center justify-center font-bold text-slate-900 dark:text-white text-xs">
											{req.name.charAt(0).toUpperCase()}
										</div>
										<div>
											<div class="text-sm font-medium text-slate-800 dark:text-slate-200">{req.name}</div>
							...
							{#if player.outgoing_friend_requests && player.outgoing_friend_requests.length > 0}
								...
								{#each player.outgoing_friend_requests as req}
									<div class="flex items-center gap-3 p-2 bg-slate-900/30 rounded-xl border border-slate-300/30 dark:border-slate-700/30 opacity-70">
										<div class="w-8 h-8 rounded-lg bg-slate-700 flex items-center justify-center font-bold text-slate-900 dark:text-white text-xs">
											{req.name.charAt(0).toUpperCase()}
										</div>
										<div>
