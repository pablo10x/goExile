# Node Error Package (Go)

A comprehensive error handling package for the Go node service that provides structured error reporting, user-friendly messages, and detailed logging.
...
This package is included in the node internal packages. Import from:

```go
import "node/internal/errors"
```
...
err := errors.Network(
    ...
    map[string]interface{}{"nodeId": "node-123"},
    &errors.ErrorContext{
        NodeID: "node-123",
        Operation: "connect",
    },
...
```go
func connectToNode(nodeId string) error {
    resp, err := http.Get(fmt.Sprintf("http://%s/api/health", nodeId))
    if err != nil {
...
func (h *Handler) handleError(c *gin.Context, err error, context *errors.ErrorContext) {
    nodeErr := errors.HandleError(err, h.logger, context)
    statusCode, response := nodeErr.ToHTTPResponse()
...
- `Network(code, message string, details map[string]interface{}, context *ErrorContext) *NodeError`
- `Validation(code, message string, details map[string]interface{}, context *ErrorContext) *NodeError`
- `Permission(code, message string, details map[string]interface{}, context *ErrorContext) *NodeError`
- `Resource(code, message string, details map[string]interface{}, context *ErrorContext) *NodeError`
- `Config(code, message string, details map[string]interface{}, context *ErrorContext) *NodeError`
- `Timeout(code, message string, details map[string]interface{}, context *ErrorContext) *NodeError`
- `Wrap(err error, errType ErrorType, context *ErrorContext) *NodeError`

### Utility Functions

- `HandleError(err error, logger *slog.Logger, context *ErrorContext) *NodeError`: Processes errors for logging
