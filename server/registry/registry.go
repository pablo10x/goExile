...
type Registry struct {
	mu     sync.RWMutex
	nextID int
	items  map[int]*models.Spawner
}

var GlobalRegistry = &Registry{
	nextID: 1,
	items:  make(map[int]*models.Spawner),
}
...
// SetItem sets a spawner item in the registry safely.
func SetItem(id int, s *models.Spawner) {
	GlobalRegistry.mu.Lock()
	defer GlobalRegistry.mu.Unlock()
...
func (r *Registry) Reset() {
	...
	defer r.mu.Unlock()
	r.nextID = 1
	r.items = make(map[int]*models.Spawner)
}
...
func (r *Registry) Register(s *models.Spawner) (int, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
	...
	if database.DBConn != nil {
		...
		// database.SaveSpawner creates if not exists.

		id, err := database.SaveSpawner(database.DBConn, s)
		if err != nil {
...
func (r *Registry) UpdateHeartbeat(id int, currentInstances, maxInstances int, status string, cpuUsage float64, memUsed, memTotal, diskUsed, diskTotal uint64, gameVersion string) error {
	...
	if database.DBConn != nil {
		if _, err := database.SaveSpawner(database.DBConn, s); err != nil {
...
func (r *Registry) Get(id int) (*models.Spawner, bool) {
	r.mu.RLock()
	defer r.mu.RUnlock()
	s, ok := r.items[id]
	return s, ok
...
func (r *Registry) Lookup(host string, port int) (*models.Spawner, bool) {
	r.mu.RLock()
	defer r.mu.RUnlock()
	...
	if database.DBConn != nil {
		s, err := database.GetSpawnerByHostPort(database.DBConn, host, port)
		if err == nil && s != nil {
			return s, true
		}
	}
...
func (r *Registry) List() []models.Spawner {
	r.mu.RLock()
	defer r.mu.RUnlock()
	out := make([]models.Spawner, 0, len(r.items))
...
func (r *Registry) All() []*models.Spawner {
	r.mu.RLock()
	defer r.mu.RUnlock()
	out := make([]*models.Spawner, 0, len(r.items))
	for _, v := range r.items {
...
func (r *Registry) Delete(id int) bool {
	...
	if database.DBConn != nil {
		if err := database.DeleteSpawnerDB(database.DBConn, id); err != nil {
