...
func (mc *MetricsCollector) CollectDatabaseMetrics() DatabaseMetrics {
	...
	metrics.Connected = GlobalStats.DBConnected
	metrics.OpenConnections = GlobalStats.DBOpenConnections
	metrics.InUse = GlobalStats.DBInUse
	metrics.Idle = GlobalStats.DBIdle
	metrics.WaitCount = GlobalStats.DBWaitCount
	metrics.WaitDurationMs = GlobalStats.DBWaitDuration.Milliseconds()
	metrics.MaxLifetimeClosed = GlobalStats.DBMaxLifetimeClosed
	metrics.MaxIdleClosed = GlobalStats.DBMaxIdleClosed
	metrics.Size = GlobalStats.DBSize
	metrics.Commits = GlobalStats.DBCommits
	metrics.Rollbacks = GlobalStats.DBRollbacks
	metrics.CacheHitRatio = GlobalStats.DBCacheHit
	metrics.TupFetched = GlobalStats.DBTupFetched
	metrics.TupInserted = GlobalStats.DBTupInserted
	metrics.TupUpdated = GlobalStats.DBTupUpdated
	metrics.TupDeleted = GlobalStats.DBTupDeleted
...
func (mc *MetricsCollector) CollectNetworkMetrics() NetworkMetrics {
	GlobalStats.mu.RLock()
	totalReqs := GlobalStats.TotalRequests
	totalErrs := GlobalStats.TotalErrors
	bytesSent := GlobalStats.BytesSent
	bytesRecv := GlobalStats.BytesReceived
	...
	activeConnections := 0
	GlobalWSManager.mu.RLock()
	activeConnections = len(GlobalWSManager.connections)
	GlobalWSManager.mu.RUnlock()
