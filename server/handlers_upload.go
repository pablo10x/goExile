...
func HandleUploadGameServer(w http.ResponseWriter, r *http.Request) {
	if dbConn == nil {
		writeError(w, r, http.StatusServiceUnavailable, "Database not connected, cannot track versions")
		return
	}

	// 1. Parse Multipart Form (100MB limit)
	if err := r.ParseMultipartForm(100 << 20); err != nil {
		writeError(w, r, http.StatusBadRequest, "File too large or invalid form")
		return
	}
	...
	if err != nil {
		writeError(w, r, http.StatusBadRequest, "Error retrieving file")
		return
	}
	defer file.Close()
	...
	if err := os.MkdirAll("./files", 0755); err != nil {
		writeError(w, r, http.StatusInternalServerError, "Failed to create directory")
		return
	}
	...
	if err != nil {
		writeError(w, r, http.StatusInternalServerError, "Failed to create file on server")
		return
	}
	defer dst.Close()
	...
	if _, err := io.Copy(dst, file); err != nil {
		writeError(w, r, http.StatusInternalServerError, "Failed to save file")
		return
	}
	...
	versions, _ := ListServerVersions(dbConn)
	if len(versions) == 0 {
		version.IsActive = true
	}

	if err := SaveServerVersion(dbConn, version); err != nil {
		// Try to cleanup file if DB insert fails
		os.Remove(dstPath)
		writeError(w, r, http.StatusInternalServerError, "Failed to save version metadata")
		return
	}
...
func ListVersions(w http.ResponseWriter, r *http.Request) {
	...
	versions, err := ListServerVersions(dbConn)
	if err != nil {
...
func HandleSetActiveVersion(w http.ResponseWriter, r *http.Request) {
	...
	if err := SetActiveVersion(dbConn, id); err != nil {
		writeError(w, r, http.StatusInternalServerError, "Failed to set active version")
		return
	}
...
func HandleDeleteVersion(w http.ResponseWriter, r *http.Request) {
	...
	active, _ := GetActiveServerVersion(dbConn)
	if active != nil && active.ID == id {
		writeError(w, r, http.StatusBadRequest, "Cannot delete the active version. Activate another version first.")
		return
	}

	filename, err := DeleteServerVersion(dbConn, id)
	if err != nil {
		writeError(w, r, http.StatusInternalServerError, "Failed to delete version from DB")
		return
	}
