...
func GenerateEnrollmentKeyHandler(w http.ResponseWriter, r *http.Request) {
	if enrollmentManager == nil {
		writeError(w, r, http.StatusInternalServerError, "enrollment manager not initialized")
		return
	}
	...
	if err != nil {
		writeError(w, r, http.StatusInternalServerError, "failed to generate enrollment key")
		return
	}
...
func ValidateEnrollmentKeyHandler(w http.ResponseWriter, r *http.Request) {
	...
	if req.Key == "" {
		writeError(w, r, http.StatusBadRequest, "enrollment key is required")
		return
	}
	...
	if !exists {
		writeError(w, r, http.StatusNotFound, "invalid enrollment key")
		return
	}

	if time.Now().After(key.ExpiresAt) {
		writeError(w, r, http.StatusGone, "enrollment key has expired")
		return
	}

	if key.Used {
		writeError(w, r, http.StatusConflict, "enrollment key has already been used")
		return
	}
...
func RevokeEnrollmentKeyHandler(w http.ResponseWriter, r *http.Request) {
	...
	if !enrollmentManager.RevokeKey(req.Key) {
		writeError(w, r, http.StatusNotFound, "enrollment key not found")
		return
	}

	writeJSON(w, http.StatusOK, map[string]string{"message": "enrollment key revoked"})
}
