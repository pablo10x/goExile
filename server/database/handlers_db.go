...
func GetTableDataHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		utils.WriteError(w, r, http.StatusInternalServerError, "failed to list tables")
		return
	}
	allowed := false
	...
	if !allowed {
		utils.WriteError(w, r, http.StatusForbidden, "access denied to table")
		return
	}
	...
	// Fetch Total Count
	var total int
	if err := DBConn.Get(&total, fmt.Sprintf(`SELECT count(*) FROM %q.%q`, schema, tableName)); err != nil {
		log.Printf("Error getting total count for table %s: %v", tableName, err)
	}

	utils.WriteJSON(w, http.StatusOK, map[string]interface{}{"data": results,
		"total":  total,
		"limit":  limit,
		"offset": offset,
	})
...
func CreateInternalBackupHandler(w http.ResponseWriter, r *http.Request) {
	...
	if output, err := cmd.CombinedOutput(); err != nil {
		utils.WriteError(w, r, http.StatusInternalServerError, fmt.Sprintf("pg_dump failed: %v, output: %s", err, string(output)))
		return
	}

	utils.WriteJSON(w, http.StatusCreated, map[string]string{"message": "backup created", "filename": filename})
}
...
func ListInternalBackupsHandler(w http.ResponseWriter, r *http.Request) {
	...
	utils.WriteJSON(w, http.StatusOK, backups)
}
...
func DownloadInternalBackupHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err := utils.ValidateFilename(filename); err != nil {
		utils.WriteError(w, r, http.StatusBadRequest, "invalid filename: "+err.Error())
		return
	}

	path := filepath.Join(backupDir, filename)
	if _, err := os.Stat(path); os.IsNotExist(err) {
		utils.WriteError(w, r, http.StatusNotFound, "backup not found")
		return
	}
...
func RestoreInternalBackupHandler(w http.ResponseWriter, r *http.Request) {
	...
	if output, err := cmd.CombinedOutput(); err != nil {
		utils.WriteError(w, r, http.StatusInternalServerError, fmt.Sprintf("restore failed: %v, output: %s", err, string(output)))
		return
	}

	utils.WriteJSON(w, http.StatusOK, map[string]string{"message": "restored successfully"})
}
...
func UpdatePostgresConfigHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err := utils.DecodeJSON(r, &req); err != nil {
		utils.WriteError(w, r, http.StatusBadRequest, "invalid json")
		return
	}

	if !isValidColumnName(req.Name) { // Reuse existing validator for parameter names
		utils.WriteError(w, r, http.StatusBadRequest, "invalid parameter name")
		return
	}
	...
	if _, err := DBConn.Exec(query); err != nil {
		utils.WriteError(w, r, http.StatusInternalServerError, fmt.Sprintf("failed to update config: %v", err))
		return
	}

	utils.WriteJSON(w, http.StatusOK, map[string]string{"message": "config updated"})
}
...
func RestartPostgresHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err := DBConn.Get(&res, "SELECT pg_reload_conf()"); err != nil {
		utils.WriteError(w, r, http.StatusInternalServerError, fmt.Sprintf("failed to reload config: %v", err))
		return
	}

	utils.WriteJSON(w, http.StatusOK, map[string]string{"message": "configuration reloaded"})
}
...
func CreateSchemaHandler(w http.ResponseWriter, r *http.Request) {
	...
	utils.WriteJSON(w, http.StatusCreated, map[string]string{"message": "schema created"})
}
...
func DeleteSchemaHandler(w http.ResponseWriter, r *http.Request) {
	...
	utils.WriteJSON(w, http.StatusOK, map[string]string{"message": "schema deleted"})
}
...
func CreateRoleHandler(w http.ResponseWriter, r *http.Request) {
	...
	utils.WriteJSON(w, http.StatusCreated, map[string]string{"message": "role created"})
}
...
func DeleteRoleHandler(w http.ResponseWriter, r *http.Request) {
	...
	utils.WriteJSON(w, http.StatusOK, map[string]string{"message": "role deleted"})
}
...
func ListTablesBySchemaHandler(w http.ResponseWriter, r *http.Request) {
	...
	if schema == "" {
		...
		utils.WriteError(w, r, http.StatusBadRequest, "schema parameter required")
		return
	}
...
func ListColumnsHandler(w http.ResponseWriter, r *http.Request) {
	...
	if schema == "" || table == "" {
		utils.WriteError(w, r, http.StatusBadRequest, "schema and table required")
		return
	}
...
func ExecuteSQLHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err := utils.IsSafeReadOnlyQuery(req.Query); err != nil {
		utils.WriteError(w, r, http.StatusForbidden, "query not allowed: "+err.Error())
		return
	}

	results, err := ExecuteSQL(conn, req.Query)
	if err != nil {
		utils.WriteError(w, r, http.StatusBadRequest, utils.SanitizeDBError(err))
		return
	}
...
func UpdateTableRowHandler(w http.ResponseWriter, r *http.Request) {
	...
	if !isValidColumnName(pkColumn) {
		utils.WriteError(w, r, http.StatusBadRequest, "invalid primary key column name")
		return
	}
	...
	if err != nil {
		utils.WriteError(w, r, http.StatusInternalServerError, "failed to get columns")
		return
	}
	...
	if !pkColumnExists {
		utils.WriteError(w, r, http.StatusBadRequest, fmt.Sprintf("primary key column '%s' does not exist in table", pkColumn))
		return
	}
	...
	if len(data) == 0 {
		utils.WriteError(w, r, http.StatusBadRequest, "no data to update")
		return
	}
	...
	for k, v := range data {
		if !isValidColumnName(k) {
			utils.WriteError(w, r, http.StatusBadRequest, "invalid column name: "+k)
			return
		}
	...
	if len(setParts) == 0 {
		utils.WriteJSON(w, http.StatusOK, map[string]string{"message": "no changes"})
		return
	}
	...
	if _, err := DBConn.Exec(query, args...); err != nil {
		utils.WriteError(w, r, http.StatusInternalServerError, utils.SanitizeDBError(err))
		return
	}

	utils.WriteJSON(w, http.StatusOK, map[string]string{"message": "row updated"})
}
...
func InsertTableRowHandler(w http.ResponseWriter, r *http.Request) {
	...
	utils.WriteJSON(w, http.StatusOK, map[string]string{"message": "row inserted"})
}
...
func DeleteTableRowHandler(w http.ResponseWriter, r *http.Request) {
	...
	utils.WriteJSON(w, http.StatusOK, map[string]string{"message": "row deleted"})
}
...
func CreateTableHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err := utils.ValidateSchemaName(req.Schema); err != nil {
	...
	for _, col := range req.Columns {
		if err := utils.ValidateColumnDefinition(col); err != nil {
			utils.WriteError(w, r, http.StatusBadRequest, "invalid column definition: "+err.Error())
			return
		}
	}
	...
	utils.WriteJSON(w, http.StatusCreated, map[string]string{"message": "table created"})
}
...
func DropTableHandler(w http.ResponseWriter, r *http.Request) {
	...
	utils.WriteJSON(w, http.StatusOK, map[string]string{"message": "table dropped"})
}
...
func AlterTableHandler(w http.ResponseWriter, r *http.Request) {
	...
	case "add_column":
		...
		if err := utils.ValidateSQLType(req.Type); err != nil {
			utils.WriteError(w, r, http.StatusBadRequest, "invalid column type: "+err.Error())
			return
		}
	...
	case "rename_column":
		...
		if err := utils.ValidateColumnName(req.NewName); err != nil {
			utils.WriteError(w, r, http.StatusBadRequest, "invalid new column name: "+err.Error())
			return
		}
	...
	case "alter_column_default":
		...
		if err := utils.ValidateDefaultValue(req.Default); err != nil {
			utils.WriteError(w, r, http.StatusBadRequest, "invalid default value: "+err.Error())
			return
		}
		err = AlterColumnDefault(DBConn, req.Schema, table, req.Column, req.Default)
	default:
		utils.WriteError(w, r, http.StatusBadRequest, "invalid action")
		return
	}
	...
	utils.WriteJSON(w, http.StatusOK, map[string]string{"message": "table altered successfully"})
}
...
func ListFunctionsHandler(w http.ResponseWriter, r *http.Request) {
	...
	utils.WriteJSON(w, http.StatusOK, functions)
}
...
func GetFunctionHandler(w http.ResponseWriter, r *http.Request) {
	...
	if oid == "" {
		utils.WriteError(w, r, http.StatusBadRequest, "oid is required")
		return
	}
	...
	utils.WriteJSON(w, http.StatusOK, fn)
}
...
func CreateFunctionHandler(w http.ResponseWriter, r *http.Request) {
	...
	if req.Name == "" || req.Returns == "" || req.Body == "" {
		utils.WriteError(w, r, http.StatusBadRequest, "name, returns, and body are required")
		return
	}
	...
	if err := utils.ValidateFunctionLanguage(req.Language); err != nil {
		utils.WriteError(w, r, http.StatusBadRequest, err.Error())
		return
	}
	if err := utils.ValidateVolatility(req.Volatility); err != nil {
		utils.WriteError(w, r, http.StatusBadRequest, err.Error())
		return
	}
	if err := utils.ValidateFunctionArguments(req.Arguments); err != nil {
		utils.WriteError(w, r, http.StatusBadRequest, err.Error())
		return
	}
	if err := utils.ValidateFunctionReturnType(req.Returns); err != nil {
		utils.WriteError(w, r, http.StatusBadRequest, err.Error())
		return
	}
	if err := utils.ValidateFunctionBody(req.Body); err != nil {
		utils.WriteError(w, r, http.StatusBadRequest, err.Error())
		return
	}
	...
	if req.SecurityDefiner {
		utils.WriteError(w, r, http.StatusForbidden, "SECURITY DEFINER functions are not allowed for security reasons")
		return
	}
	...
	utils.WriteJSON(w, http.StatusCreated, map[string]string{"message": "function created successfully"})
}
...
func UpdateFunctionHandler(w http.ResponseWriter, r *http.Request) {
	...
	utils.WriteJSON(w, http.StatusOK, map[string]string{"message": "function updated successfully"})
}
...
func DeleteFunctionHandler(w http.ResponseWriter, r *http.Request) {
	...
	if req.Name == "" {
		utils.WriteError(w, r, http.StatusBadRequest, "function name is required")
		return
	}
	...
	utils.WriteJSON(w, http.StatusOK, map[string]string{"message": "function dropped successfully"})
}
