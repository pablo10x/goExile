...
func MigrateDB(db *sqlx.DB) error {
	...
	queries := []string{fmt.Sprintf(`CREATE TABLE IF NOT EXISTS nodes (
                id %s,
                name TEXT,
                region TEXT,
                host TEXT NOT NULL,
...
func GetNotes(db *sqlx.DB) ([]models.Note, error) {
	...
	for rows.Next() {
		...
		var title, content, color, status sql.NullString

		if err := rows.Scan(&n.ID, &title, &content, &color, &status, &n.Rotation, &tsUnixCreated, &tsUnixUpdated); err != nil {
...
func SaveNode(db *sqlx.DB, s *models.Node) (int, error) {
	if s == nil {
...
func DeleteNodeDB(db *sqlx.DB, id int) error {
	do := func() error {
...
func SaveInstanceAction(db *sqlx.DB, a *models.InstanceAction) (int, error) {
	var id int
	do := func() error {
		query := `INSERT INTO instance_actions (node_id, instance_id, action, timestamp, status, details) VALUES ($1, $2, $3, $4, $5, $6) RETURNING id`
		err := db.QueryRow(query, a.NodeID, a.InstanceID, a.Action, a.Timestamp.Unix(), a.Status, a.Details).Scan(&id)
		if err != nil {
...
func GetInstanceActions(db *sqlx.DB, nodeID int, instanceID string) ([]models.InstanceAction, error) {

	rows, err := db.Queryx(`SELECT id, node_id, instance_id, action, timestamp, status, details FROM instance_actions WHERE node_id = $1 AND instance_id = $2 ORDER BY timestamp DESC LIMIT 50`, nodeID, instanceID)
	if err != nil {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&a.ID, &a.NodeID, &a.InstanceID, &a.Action, &tsUnix, &status, &details); err != nil {
...
func SeedDefaultConfig(db *sqlx.DB) error {
	defaultConfigs := []models.ServerConfig{
		...
		},
		{
			...
			Type:            "duration",
			Category:        "system",
			Description:     "How long a node is considered alive since last heartbeat",
			IsReadOnly:      false,
		...
		},
		{
			...
			Type:            "duration",
			Category:        "system",
			Description:     "Frequency of cleanup routine for inactive nodes",
			IsReadOnly:      false,
			RequiresRestart: false,
			UpdatedBy:       "system",
		},
		...
		{
			Key:             "max_instances_per_node",
			Value:           "20",
			Type:            "int",
			Category:        "node",
			Description:     "Default maximum instances per node",
			IsReadOnly:      false,
			RequiresRestart: false,
			UpdatedBy:       "system",
		},
		...
		},
		{
			...
			Type:            "string",
			Category:        "aesthetic",
			Description:     "Active site theme (light/dark)",
			IsReadOnly:      false,
			RequiresRestart: false,
			UpdatedBy:       "system",
		},
...
func GetAllConfig(db *sqlx.DB) ([]models.ServerConfig, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&c.ID, &c.Key, &c.Value, &c.Type, &c.Category, &description, &c.IsReadOnly, &c.RequiresRestart, &updatedAtUnix, &updatedBy); err != nil {
...
func GetBannedIPsFull(db *sqlx.DB) ([]models.RedEyeIPReputation, error) {
	...
	for rows.Next() {
		...
		var isBannedInt int
		var banReason sql.NullString
		if err := rows.Scan(&r.IP, &r.ReputationScore, &r.TotalEvents, &tsUnix, &isBannedInt, &banReason, &banExpiresUnix); err != nil {
			return nil, err
		}
		r.LastSeen = time.Unix(tsUnix, 0).UTC()
		r.IsBanned = isBannedInt == 1
